...
def stream_segmentation(stream, smooth_range=1e3, tvd_lamda=1000):
    ...
    for s in stream_list:
        if 'chi' in s.dataset and 'elevation' in s.dataset:
            filter_window_length = int(smooth_range/(s.length()/len(s.dataset['flow_order'])))
            if filter_window_length > len(s.dataset['flow_order']):
                filter_window_length = len(s.dataset['flow_order'])
            if filter_window_length%2 == 0:
                filter_window_length -= 1
            segment, ksn_segmented = _stream_segmentation_impl(
                s.dataset['chi'].data, s.dataset['elevation'].data,
                filter_window_length, tvd_lamda)
            s.dataset['segment'] = (['flow_order'], segment)
            s.dataset['ksn_segmented'] = (['flow_order'], ksn_segmented)
        else:
            raise ValueError("Missing 'chi' or 'elevation' in `Stream.dataset`.")
...
#@_speed_up
def _stream_segmentation_impl(chi: np.ndarray, elev: np.ndarray, filter_window_length, tvd_lamda):
    ...
    ksn_segmented = TVD(ksn, tvd_lamda)

    segment = np.zeros_like(ksn_segmented)

    for i in range(1, len(segment)):
        if ksn_segmented[i] == ksn_segmented[i-1]:
            segment[i] = segment[i-1]
        else:
            segment[i] = segment[i-1] + 1

    return segment, ksn_segmented
